...
function afficheType(){
	echo '<div class ="span9"><p class ="arborescence muted">';
			...
			if ($compteur !=0){ 
				...
				while ($row = mysql_fetch_array($result)) {	
					...
					echo '<div class="btn-group "> <a class="btn btn-info btn-liste" href="liste.php?type='.$row["id"].'">'.$row["nom"].' </a></div>';
						...
						echo '<a class="btn btn-info dropdown-toggle btn-liste" data-toggle="dropdown" href="#">';
						echo $row["nom"];
...
function afficheListe(){
	...
	if(isset($_POST["recherche"])){
		...
		if ($compteur !=0){
			...
			echo '<div class ="span9"><p class = "arborescence"> '.$compteur.' résultats</p></div><div  <div class ="span7 well content" id = "content">';
			...
			while ($row = mysql_fetch_array($result)) {	
				...
				<div class = "offset1 span9 well"> 
					<div > 		
						...
						if (isset($url[1])){
							echo '<div class ="span2"> <img src="http://s.wordpress.com/mshots/v1/http%3A%2F%2F'.$url[1].'?w=200&h=125" alt="Image" class = "frame">';
						}
						else{
							echo '<div class ="span2 "> Pas de preview';				
						}
						
						$sql_note = "SELECT * FROM cat_note N, cat_ressource R WHERE "; 
						$sql_note .= "R.id = ".$row['id']." AND note_id = N.id" ;
						$result_note = mysql_query($sql_note);
						while ($row_note = mysql_fetch_array($result_note)) {
							$nb_votant = $row_note["nb_votant"];
							if ($nb_votant==0)
								$nb_votant =1;
							$note = round($row_note["note"]/$nb_votant);
						}
						echo '<div class=""><ul class="notes-echelle">';
						for ($i=1; $i<=5; $i++){
							echo '<li>';
							echo '<label for="note0'.$i.'" title="Note&nbsp;: '.$i.' sur 5">`</label>';
							if ($i == $note){
								echo '<input type="radio" name="notesA" id="note0'.$i.'" value="'.$i.'" checked="checked" />';					
							}							
							else {
								echo '<input type="radio" name="notesA" id="note0'.$i.'" value="'.$i.'" />';
							}
						}
						echo '</ul></div>';
					...
					<div class="ressource span3"> 
						...
						<li> <strong>URL : </strong><?php echo preg_replace ('#(ht{2}ps?:/{2}[^[:blank:]]+)#i', '<a href="\1" target="_blank" title="Aller à l\'URL \1 (nouvelle fenêtre)">\1</a>', $row["url"]);	?></li> 	
						<li> <strong>Auteur : </strong><?php echo $row["auteur"]?></li> 			
						<li> <strong>Version : </strong><?php echo $row["version"]?></li> 
						<li> <strong>Ressources liées au fonctionnement : </strong> <br/>
						<?php 
						...
						?></li> 	
						<li> <strong>Ressources analogues : </strong> <br/><?php 						
...
function afficherRecherche(){  
	...
	if (empty($result)){
		...
		foreach ($result as $ressource){
			...
			if (isset($url[1])){
				echo '<div class="liste row"><div class ="span2"> <img src="http://s.wordpress.com/mshots/v1/http%3A%2F%2F'.$url[1].'?w=200&h=125" alt="Image" class = "frame"></div>';
			}
